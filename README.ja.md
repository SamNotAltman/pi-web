...
| `PI_WEB_PASSWORD` | ログインページとパスキー登録を有効化 | 認証なし |
| `PI_WEB_SHOW_PASSWORD_LOGIN` | パスキーログイン画面に「パスワードでサインイン」ボタンを表示 | 非表示 |
...
`PI_WEB_PASSWORD` を設定すると、パスキーを登録し、パスワードの代わりにデバイスの指紋・顔認証・セキュリティキーでサインインできます。パスワードは初回登録とリカバリのための控えとして残ります：

1. まずパスワードで一度サインインします。
2. 「設定 → Passkeys」を開き、**Add passkey** を選びます。
3. 以降は **Sign in with passkey** が中心になり、パスワードはフォールバックとして残ります。

パスキーは WebAuthn API を使うため、ブラウザはセキュアコンテキストでのみ公開します。`http://localhost` と `http://127.0.0.1` はセキュアコンテキストですが、通常の HTTP の LAN アドレスはそうではありません。`http://192.168.x.x` ではログインページがパスキーボタンを隠し、パスワードは引き続き動作します。リモートでパスキーを使うには、プロキシで HTTPS を終端し、外部ホスト名を `PI_WEB_ALLOWED_HOSTS` に追加してください。資格情報は `~/.pi/agent/pi-web-passkeys.json` に保存され、登録時のホスト名に紐づくため、別のホスト名では使用できません。
