...
test("shows the password fallback button only for opt-in values", async () => {
  const { isPasswordLoginVisible } = await loadSubject();
  assert.equal(isPasswordLoginVisible(undefined), false);
  assert.equal(isPasswordLoginVisible(""), false);
  assert.equal(isPasswordLoginVisible("0"), false);
  assert.equal(isPasswordLoginVisible("false"), false);
  assert.equal(isPasswordLoginVisible("1"), true);
  assert.equal(isPasswordLoginVisible("true"), true);
  assert.equal(isPasswordLoginVisible("YES"), true);
});
