...
| `PI_WEB_PASSWORD` | Включить вход и регистрацию passkey | Аутентификация отключена |
| `PI_WEB_SHOW_PASSWORD_LOGIN` | Показывать кнопку «Войти с паролем» на экране входа с passkey | Скрыто |
...
Если задан `PI_WEB_PASSWORD`, можно зарегистрировать passkey и входить по отпечатку, лицу или аппаратному ключу вместо ввода пароля. Пароль остаётся резервом для первичной регистрации и восстановления:

1. Войдите по паролю один раз.
2. Откройте «Настройки → Passkeys» и выберите **Добавить passkey**.
3. При следующих визитах основным способом становится **Sign in with passkey**, а пароль остаётся запасным.

Passkeys используют WebAuthn API, который браузеры предоставляют только в безопасном контексте. `http://localhost` и `http://127.0.0.1` считаются безопасными, а обычный HTTP-адрес в локальной сети — нет: по `http://192.168.x.x` страница входа скрывает кнопку passkey, а пароль продолжает работать. Для удалённых passkeys завершайте HTTPS на прокси и добавьте внешнее имя хоста в `PI_WEB_ALLOWED_HOSTS`. Учётные данные хранятся в `~/.pi/agent/pi-web-passkeys.json` и привязаны к имени хоста регистрации, поэтому passkey одного хоста не работает на другом.
